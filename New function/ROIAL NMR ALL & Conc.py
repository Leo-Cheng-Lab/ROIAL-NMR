...
def define_region(region):
    ...
    for m in range(0, len(region_table0)):
        ...
        cp_region1 = [x for x in list(region_table0[m]) if pd.isnull(x) == False][1:] #get rid of the 'nan' in the table
        for j in range(0, len(cp_region1)):    
            ...
            out_range = 1
            for h in range(0, len(low_region)):
                if low_region[h] <= single_region1[0] < upp_region[h]  or low_region[h] < single_region1[1] <= upp_region[h]:
                    out_range = 0
            if out_range == 1:
...
def output_f_table_Conc(tab):
    ...
    df["concentration range"] = pd.Categorical(
        df["concentration range"],
        categories=order,
        ordered=True
    )

    df = df.sort_values("concentration range")
    df = df.iloc[:-1]
    # 输出 Excel
    output_file = filename_categorize
...
def categorize_in_regions(category_dicts):#list the metabolites with their catagory in each ppm region
    ...
    for h in range(0,imp_data.shape[0]):
        ...
        for i in output_dict:
            for m in range(0,abbreviation_table.shape[0]):
                if i == abbreviation_table[m,0]:
                    if abbreviation_table[m,1] != " ":
                        i = abbreviation_table[m,1]
...
def categorize_in_groups(dict_f):#categroized the metabolites based on the group and trend
    ...
    for x in range(0,3):# write the groups in order: FDR-signifi goes first, then significant and the last is not significant
        ...
        for i in de_seq_list: 
           ...
           if len(decrease_list) == 4: # 4 metabolites in one row
               ...
               decrease_list = []
               row_number += 1
           if len(decrease_list) == len(dict_f [de_group_list[x]])-4*(row_number-layer_number) and len(decrease_list) != 0:#row number less than 4
    ...
    for x in range(0,3):
        ...
        '''
           if len(increase_list) == 4: # 4 metabolites in one row
               ...
               increase_list = []
               row_number += 1
           if len(increase_list) == len(dict_f [in_group_list[x]])-4*(row_number-layer_number) and len(increase_list) != 0:
...
def UI_search():
    ...
    for n in range(0, imp_data.shape[0]):
        ...
        for j in range(0, len(output_dict)):
            output_str = str(output_str) + list(output_dict.keys())[j] + ' ' + str(sorted(set(list(output_dict.values())[j]))) + "\n"
        if output_str == '':
            output_str = 'none'
    ...
    for i in range (0,len(list(uq_cps_dict2.keys()))):
        ...
        if cp_index != None:
            cp_row = cp_index[0]
            cp_region = [x for x in list(region_table[cp_row]) if pd.isnull(x) == False][1:] #get rid of the 'nan' in the table
            cp_region = list(set(cp_region))
            cp_region0 =  [x for x in list(region_table0[cp_row]) if pd.isnull(x) == False][1:] #get rid of the 'nan' in the table
            cp_region0 = list(set(cp_region0))
            bottom_pass = 0
            if frac(len(cp_region0), len(cp_region)) > 0:# if more than [certain ratio] of the metabolites regions are removed, the metabolites will not be identified 
                bottom_pass = 1
            ...
            if bottom >= 1 and bottom_pass == 1:
                for n in range(0,bottom):#we found the ppm appears first, and according to the ppm we can calculate how many region appears
                    single_region = cp_region0[n].replace('[',',').replace(']',',').split(',')[1:-1]#get rid of the '[]'
            ...
            str_fraction = str(top) + '/' + str(bottom)# the match ratio is based on the region shows up instead of the ppm
            '''
            match ratio select standard
            '''
            if fraction >= 0.0: #and conc_gory != "<5 uM": the match ratio should be 50% or greater
    ...
    f_table = format_f_table(f_table2)
    ...
    if ans.strip().lower() == 'y': # if they have a priority list
        if action.strip() == '1':
            ...
            filename = input('Priority Metabolites List excel filename: ')
        ...
        for i in range(0,len(priority_cp_list)):
            for j in range(0, len(output_dict2)):
                if  priority_cp_list[i].strip().lower() == list(output_dict2.keys())[j].strip().lower():
                    output_dict3.update({list(output_dict2.keys())[j]:output_dict2[list(output_dict2.keys())[j]]})#add the metabolite in both output_dict2 and priority_cp_list to the output_dict3
        for i in range(0, len(priority_cp_list)):
            row = [] 
...
#UI
#file_location = input('\npackage location (e.g. D:\ROIAL NMR): ')
file_location = 'D:\Professor Cheng group\Metabolites searching program\ROIAL NMR 10.17'
...
database = input("Select database (csf, feces, saliva, serum, sweat, urine): ")
...
filename = input('\nImport ROIs excel filename: ')+'.xlsx'
path = os.path.abspath(filename)
df = pd.read_excel(path)
imp_data = np.asarray(df) 

# define which region is analysed
covered_region = input("Please input the region to be analysed (If there are multiple regions please split with';', for example 8.5-8.0; 5.5-0.5): ")
...
print('(Press 3): All metabolites with Conc sequences')
category_type= input("Please choose one format to output the result:")



ans = input("\ndisease relevant metabolites list? (y/n): ")
if ans.strip().lower() == 'y': # if they have a priority list
...
filename_categorize = input('\nThe filename of the result: ')+ '.xlsx'
UI_search()

'''
df = pd.DataFrame(f_table, columns=['Column1', 'Column2','1','1','1'])
